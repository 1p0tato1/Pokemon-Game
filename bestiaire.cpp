...
void printBestiaire(EspecePokemon bestiaire[150]) {
    for (int i = 0; i < 150; i++) {
        if (bestiaire[i].nom != nullptr) {
            cout << i << endl;
            cout << "Nom : " << bestiaire[i].nom << endl;
            cout << "Type : " << bestiaire[i].type << endl;
            if (bestiaire[i].evolvesTo != nullptr) {
                cout << "Evolution : " << bestiaire[i].evolvesTo << endl;
            }
            else {
                cout << "Evolution : Aucune" << endl;
            }
            cout << "Bonbons necessaires : " << bestiaire[i].nbBonbonsPourEvoluer << endl;
            cout << "--------------------------" << endl;
        }
    }
}
   ...
   // Trier le tableau 
    for (int i = 0; i < 149; i++) {
        ...
        for (int j = i + 1; j < 150; j++) {
            if (strcmp(bestiaireTrie[j].nom, bestiaireTrie[petit].nom) < 0) {
                petit = j;
            }
        }
        if (petit != i) {
            EspecePokemon tmp = bestiaireTrie[i];
            bestiaireTrie[i] = bestiaireTrie[petit];
            bestiaireTrie[petit] = tmp;
        }
    }
    printBestiaire(bestiaireTrie);
}
